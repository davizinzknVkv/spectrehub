...
function WelcomeTour() {
  ...
  const steps = [
    ...
    },
    {
      ...
      content: (
        <>
          Com o <span className="text-[#ff0055]">Nicks-Gun</span>, você monitora e captura usernames raros de 2 e 3 letras instantaneamente. Seja o primeiro a garantir nomes que definem status.
        </>
      ),
      button: "próximo →",
    },
      ...
      <div className="relative w-full max-w-md animate-in fade-in zoom-in duration-300">
        <div className="overflow-hidden rounded-none border border-white/10 bg-[#050505] shadow-2xl shadow-black/80">
          ...
          <div className="p-8 pb-6">
            <div className="mb-4 flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-[#ff0055]">
            ...
            <h2 className="mb-4 font-display text-2xl font-bold tracking-tight text-white">
              {steps[step].title.includes('Spectre Hub') ? (
                <>
                  O que é o <span className="text-white">Spectre<span className="text-[#ff0055]">hub</span></span>?
                </>
              ) : steps[step].title}
            </h2>
              ...
              {steps.map((_, i) => (
                <div
                  ...
                  className={`h-1 rounded-full transition-all duration-300 ${
                    i === step ? "w-6 bg-[#ff0055]" : "w-1.5 bg-white/10"
                  }`}
                />
              ))}
            </div>
